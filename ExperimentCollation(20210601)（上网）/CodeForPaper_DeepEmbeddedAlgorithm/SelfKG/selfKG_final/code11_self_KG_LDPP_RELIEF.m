...
%                         svm_pred = svmpredict(validY,validZ,model,'-b 1');  % 这里的testZ还是valid
%                         svml8(ik)= mean(svm_pred == validY) * 100;
% 
%                        %# SVM高斯 
...
%                  
%                  model = svmtrain(trainY,trainZ2,'-s 0 -t 0 -b 1');%%使用所有变换后的训练集训练模型
%                  svm_pred = svmpredict(validY,validZ2,model,'-b 1');
...
% %                  svml2 = [svml2 mean(svm_pred == validY) * 100]; 
% %                  
% %                  model = svmtrain(trainY,trainZ2,'-s 0 -t 2 -b 1 ');%%使用所有变换后的训练集训练模型
% %                  svm_pred = svmpredict(validY,validZ2,model,'-b 1');
% %                  svml2 = [svml2 mean(svm_pred == validY) * 100];
% %                  
%               % RF随机森林
...
% %            
%            model = svmtrain(trainY,train,'-s 0 -t 0 -b 1');%%使用所有变换后的训练集训练模型
%            svm_pred1 = svmpredict(testY,test,model,'-b 1');
%            svml3 = [svml3 mean(svm_pred1 == testY) * 100];
% 
%             % SVM高斯 
...
% %              svml3 = [svml3 mean(svm_pred1 == testY) * 100]; 
%              
% %              model = svmtrain(trainY,train,'-s 0 -t 2 -b 1');%%使用所有变换后的训练集训练模型
% %              svm_pred1 = svmpredict(testY,test,model,'-b 1');
% %              svml3 = [svml3 mean(svm_pred1 == testY) * 100]; 
% 
%          % RF随机森林
...
%                 svml3 = [svml3 calculateAccuracy(test_data,testY,svm_pred1)];
       ...
       Prei(s,:) = TPi(s,:) / (TPi(s,:) + FPi(s,:));
       Reci(s,:) = TPi(s,:) / (TPi(s,:) + FNi(s,:));
       Spei(s,:) = TNi(s,:) / (FPi(s,:) + TNi(s,:)); %# 修正spe指标
