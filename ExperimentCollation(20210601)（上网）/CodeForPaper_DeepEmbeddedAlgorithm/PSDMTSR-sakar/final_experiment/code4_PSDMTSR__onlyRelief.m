...
iter = 10
for C = 1:iter
    [only_Relief_Resul] = onlyRelief()
    final_eval{C} = only_Relief_Resul;
end
...
function [onlyRelief_Result] = onlyRelief()
        % 记住RF时不返回AUC
...
%        
       ...
       [svm_pred1,~,Scores] = svmpredict(testY,test,mode2);
       svml3 =  [svml3 calculateAccuracy(test_data,testY,svm_pred1)];
...
%      AUCi{s} = plot_roc(Mean_scores{s,1}(:, 2),testY);  % Mean_scores{s,2}要注意输入的哪一列的概率值。
      ...
      Finallacc =  calculateAccuracy(test_data,testY,best_Pred);
...
%    Final_Evaluation_index = [ACC Pre Rec Spe G_mean  F1_score  ] 
   ...
   onlyRelief_Result.ACC_final_mean = ACC_final_mean;
   onlyRelief_Result.ACC = ACC;   
   onlyRelief_Result.ACCi = ACCi; 
   onlyRelief_Result.Pre = Pre;
   onlyRelief_Result.Prei = Prei;
   onlyRelief_Result.Rec = Rec; 
   onlyRelief_Result.Reci = Reci;
   onlyRelief_Result.Spe = Spe; 
   onlyRelief_Result.Spei = Spei;
   onlyRelief_Result.G_mean = G_mean; 
   onlyRelief_Result.G_meani = G_meani; 
   onlyRelief_Result.F1_score = F1_score; 
   onlyRelief_Result.F1_scorei = F1_scorei; 
 
   onlyRelief_Result.svml3 = svml3; 
end