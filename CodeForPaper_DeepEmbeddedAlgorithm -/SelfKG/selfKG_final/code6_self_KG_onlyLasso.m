...
iter = 10
for C = 1:iter
    [MaxLittlerProposed_H_experiment1_s2] = Lasso()
...
function [onlyLasso_Result] = Lasso()
        % 记住RF时不返回AUC
...
%            kk = 5;
%            svml2 = [];
%            fea = sub_pvalue_selection(trainX,trainY);
%            %% 下面代码 依次增加特征列数进行训练并预测精度  
%             for ik = 1:floor(size(trainX,2)/5) 
%                  K = kk * ik;  
%                  trainZ = trainX(:,fea(:,1:K));
%                  validZ = validX(:,fea(:,1:K));
%             % SVM高斯 
...
% %        [svm_pred1,~,Scores] = svmpredict(testY,test,mode2,'-b 1');
...
%      [svm_pred1,votes] = classRF_predict(test,mode2)
%      svml3 =  [svml3 mean(svm_pred1 == testY) * 100];
     %%
%% 使用Lasso正则化特征选择算法，使用验证集选参数
             svml2 = [];
             [b fitinfo] = lasso2(trainX,trainY,'CV',10);%lasso(train_lab_rand(:,1:32),train_lab_rand(:,33),'CV',10);
             lam = fitinfo.IndexMinMSE; % find index of suggested lambda
             weight = b(:,lam)';
             in_lasso = find(weight~=0);
%              best_svml_kk = size(in_lasso,2);
             trainZ = trainX(:,in_lasso);
             validZ = validX(:,in_lasso);

  
            % SVM高斯分类器训练模型
%              model = svmtrain(trainY,trainZ,'-s 0 -t 2 ')%%使用所有变换后的训练集训练模型
%              svm_pred = svmpredict(validY,validZ,model)
%              svml2 = [svml2 calculateAccuracy(validZ,validY,svm_pred)]
...
%               svml2 = mean(svm_pred == validY) * 100;
            ...
            trainZ1 = trainX(:,in_lasso);
            testZ1 = testX(:,in_lasso);
...
%        svml3 = [svml3 mean(svm_pred1 == testY) * 100;]
       ...
       % SVM 线性 
       mode2 = svmtrain(trainY,trainZ1,'-s 0 -t 0 -b 1'); 
       [svm_pred1,~,Scores ]= svmpredict(testY,testZ1,model,'-b 1');
       svml3 = [svml3 mean(svm_pred1 == testY) * 100;];
...
%    Final_Evaluation_index = [ACC Pre Rec Spe G_mean  F1_score_Final  AUC_final ] 
   ...
   onlyLasso_Result.ACC_final_mean = ACC_final_mean;
   onlyLasso_Result.ACC = ACC;   
   onlyLasso_Result.ACCi = ACCi; 
   onlyLasso_Result.Pre = Pre;
   onlyLasso_Result.Prei = Prei;
   onlyLasso_Result.Rec = Rec; 
   onlyLasso_Result.Reci = Reci; 
   onlyLasso_Result.TNR = TNR;
   onlyLasso_Result.TNRi = TNRi

   onlyLasso_Result.Spe = Spe; 
   onlyLasso_Result.Spei = Spei;
   onlyLasso_Result.G_mean = G_mean; 
   onlyLasso_Result.G_meani = G_meani; 
   onlyLasso_Result.F1_score = F1_score; 
   onlyLasso_Result.F1_scorei = F1_scorei; 
 
   onlyLasso_Result.svml3 = svml3; 
end