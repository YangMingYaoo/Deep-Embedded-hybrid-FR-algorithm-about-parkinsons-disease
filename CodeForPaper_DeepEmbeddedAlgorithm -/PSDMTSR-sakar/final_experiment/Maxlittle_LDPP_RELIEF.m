...
% iter = 1
% for C = 1:iter
%     [MaxLittlerProposed_H_experiment1_s2] = LDPPRelief()
...
%     %%  两种方式
%       %% LDPP no_t
% function [LDPPrelief_Result] = LDPPRelief()
% % 记住RF时不返回AUC
...
% 
%             % SVM高斯 
% %              model1 = svmtrain(trainY,trainZ2,'-s 0 -t 2 ');%%使用所有变换后的训练集训练模型
% %              svm_pred2 = svmpredict(validY,validZ2,model1);
% % %               svml2 = [svml2 calculateAccuracy(valid_data,validY,svm_pred2)];
% %               svml2 = [svml2  mean(svm_pred2 == validY) * 100];
% 
%            
%               % SVM 线性
%              model1 = svmtrain(trainY,trainZ2,'-s 0 -t 0');%%使用所有变换后的训练集训练模型
%              svm_pred2 = svmpredict(validY,validZ2,model1);
% %               svml2 = [svml2 calculateAccuracy(valid_data,validY,svm_pred2);];
%            svml2 = [svml2  mean(svm_pred2 == validY) * 100];
% 
%             % RF随机森林
% %            model1 = classRF_train(trainZ2,trainY,'ntree',300)
% %            [svm_pred2,votes] = classRF_predict(validZ2,model1)
% %            svml2 = [svml2 calculateAccuracy(valid_data,validY,svm_pred2);];
% %            svml2 = [svml2  mean(svm_pred2 == validY) * 100];
%            
%             end
...
%             % SVM高斯 
% % 
% %               model2 = svmtrain(trainY,train,'-s 0 -t 2');%%使用所有变换后的训练集训练模型
% %               svm_pred1 = svmpredict(testY,test,model2);
% % %               svml3 = [svml3 calculateAccuracy(test_data,testY,svm_pred1)];
% %             svml3 = [svml3  mean(svm_pred1 == testY) * 100];   
% 
% %                % SVM 线性
%               mode2 = svmtrain(trainY,train,'-s 0 -t 0');%%使用所有变换后的训练集训练模型
%               svm_pred1 = svmpredict(testY,test,mode2);
% %               svml3 = [svml3 calculateAccuracy(test_data,testY,svm_pred1)];
%                          svml3 = [svml3  mean(svm_pred1 == testY) * 100];   
% 
% 
%                 % RF随机森林
% %                  model2 = classRF_train(train,trainY,'ntree',300)
% %                  [svm_pred1,votes] = classRF_predict(test,model2)
% %                 svml3 = [svml3 calculateAccuracy(test_data,testY,svm_pred1)];
% %                 svml3 = [svml3  mean(svm_pred1 == testY) * 100];   
% 
%        %% 每一个子空间的模型评价指标
...
%    LDPPrelief_Result.ACC_final_mean = ACC_final_mean;
%    LDPPrelief_Result.testACC = svml2 ;
%    LDPPrelief_Result.ACC = ACC;   
%    LDPPrelief_Result.ACCi = ACCi; 
%    LDPPrelief_Result.Pre = Pre;
%    LDPPrelief_Result.Prei = Prei;
%    LDPPrelief_Result.Rec = Rec; 
%    LDPPrelief_Result.Reci = Reci; 
%    LDPPrelief_Result.Spe = Spe; 
%    LDPPrelief_Result.Spei = Spei;
%    LDPPrelief_Result.G_mean = G_mean; 
%    LDPPrelief_Result.G_meani = G_meani; 
%    LDPPrelief_Result.F1_score = F1_score; 
%    LDPPrelief_Result.F1_scorei = F1_scorei; 
%    LDPPrelief_Result.svml3 = svml3; 
%    
% end
...
%               svml2 = [svml2 calculateAccuracy(valid_data,validY,svm_pred);];
         ...
         svml3 = [];
           ...
           %% 使用test进行验证
            % SVM高斯 
% 
              model = svmtrain(trainY,train,'-s 0 -t 2');%%使用所有变换后的训练集训练模型
              svm_pred1 = svmpredict(testY,test,model);
%                svml3 = [svml3 calculateAccuracy(test_data,testY,svm_pred1);];
                    svml3 = [svml3   mean(svm_pred1 == testY) * 100];
                    
               % SVM 线性
%               model = svmtrain(trainY,train,'-s 0 -t 0');%%使用所有变换后的训练集训练模型
%               svm_pred1 = svmpredict(testY,test,model);
%                svml3 = [svml3 calculateAccuracy(test_data,testY,svm_pred1);];
           
                % RF随机森林
%                  model = classRF_train(train,trainY,'ntree',300)
%                  [svm_pred1,votes] = classRF_predict(test,model)
%                 svml3 = [svml3 calculateAccuracy(test_data,testY,svm_pred1);];
