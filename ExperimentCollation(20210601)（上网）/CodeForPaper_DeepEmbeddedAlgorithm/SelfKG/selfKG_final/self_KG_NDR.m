...
%            svml2 =[svml2 mean(svm_pred == validY) * 100];
      ...
      %% 使用所有变换后的训练集训练模型并使用变换后的测试集合预测数据
       % SVM 高斯 
       mode2 = svmtrain(trainY,trainX,'-s 0 -t 2 '); 
       [svm_pred1,~,Scores] = svmpredict(testY,testX,mode2);
...
%        svml3 =  [svml3 mean(svm_pred1 == testY) * 100];
%        [svm_pred1,~,Scores] = svmpredict(validY,validX,mode2,'-b 1');
%        svml3 =  [svml3 mean(svm_pred1 == validY) * 100];
       % RF 
