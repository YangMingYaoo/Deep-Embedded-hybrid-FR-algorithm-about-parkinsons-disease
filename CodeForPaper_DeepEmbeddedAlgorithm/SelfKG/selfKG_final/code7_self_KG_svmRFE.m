...
iter = 10
for C = 1:iter
    [MaxLittlerProposed_H_experiment1_s2] = svm_RFE()
...
function [svm_RFE_Result] = svm_RFE()
        % 记住RF时不返回AUC
        clear all;
        close all ;
        clc;
         %% 加载数据集 
%         load('PD_speech_featur');%数据集读取
...
%              svml2 = [svml2 mean(svm_pred == validY) * 100;]; 
          ...
          [svm_pred,votes] = classRF_predict(validZ,model)
           ...
           [acc_svml_max,indx2] = max(svml2);
            ...
            [fea] = SVM_RFE(trainX,trainY, best_svml_kk); 
...
%        svml3 =  [svml3 mean(svm_pred1 == testY) * 100];
     ...
     svml3 =  [svml3 mean(svm_pred1 == testY) * 100];
       ...
       Spei(s,:) = TNi(s,:) / (FPi(s,:) + TNi(s,:)); %#修正每spe 
...
%    Final_Evaluation_index = [ACC Pre Rec Spe G_mean  F1_score_Final  AUC_final ] 
   ...
   svm_RFE_Result.ACC_final_mean = ACC_final_mean;
   svm_RFE_Result.ACC = ACC;   
   svm_RFE_Result.ACCi = ACCi; 
   svm_RFE_Result.Pre = Pre;
   svm_RFE_Result.Prei = Prei;
   svm_RFE_Result.Rec = Rec; 
   svm_RFE_Result.Reci = Reci; 
   svm_RFE_Result.Spe = Spe; 
   svm_RFE_Result.Spei = Spei;
   svm_RFE_Result.G_mean = G_mean; 
   svm_RFE_Result.G_meani = G_meani; 
   svm_RFE_Result.F1_score = F1_score; 
   svm_RFE_Result.F1_scorei = F1_scorei; 
 
   svm_RFE_Result.svml3 = svml3; 
end