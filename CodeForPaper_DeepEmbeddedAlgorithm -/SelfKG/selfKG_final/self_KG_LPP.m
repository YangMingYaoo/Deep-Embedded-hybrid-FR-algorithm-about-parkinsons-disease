...
iter = 10
for C = 1:iter
    [MaxLittlerProposed_H_experiment1_s2] = lpp()
...
%                svm_pred = svmpredict(validY,validZ1,model);  
%                svml8(it,ik) = mean(svm_pred == validY) * 100;
...
%                    [svm_pred1,~,Scores] = svmpredict(testY,testZ2,mode2,'-b 1');
...
%                    svml3 = [svml3 mean(svm_pred1 == testY) * 100;];
                 ...
                 [svm_pred1,votes] = classRF_predict(testZ2,mode2)
                 svml3 = [svml3 mean(svm_pred1 == testY) * 100;];
...
%    Final_Evaluation_index = [ACC Pre Rec Spe G_mean  F1_score_Final  AUC_final ] 
   ...
   lpp_Result.ACC_final_mean = ACC_final_mean;
   lpp_Result.ACC = ACC;   
   lpp_Result.ACCi = ACCi; 
   lpp_Result.Pre = Pre;
   lpp_Result.Prei = Prei;
   lpp_Result.Rec = Rec; 
   lpp_Result.Reci = Reci; 
   lpp_Result.TNR = TNR;
   lpp_Result.TNRi = TNRi

   lpp_Result.Spe = Spe; 
   lpp_Result.Spei = Spei;
   lpp_Result.G_mean = G_mean; 
   lpp_Result.G_meani = G_meani; 
   lpp_Result.F1_score = F1_score; 
   lpp_Result.F1_scorei = F1_scorei; 
 
   lpp_Result.svml3 = svml3; 
end