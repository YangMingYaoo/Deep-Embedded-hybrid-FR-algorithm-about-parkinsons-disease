...
function [LdppRelief_adaboost_Result] = LdppRelief_Adaboost()
    ...
    for z = 1 : iter
        a = []
        [fea,U,mode2,indx,misX,misY] =  LDPP_Relief_adbstLDPP1(trainX1,trainY1,validX,validY,valid_data,type_num)
        Us{z}=U;
        feature{z,1}=fea;
        Medels{z,1} = mode2;
...
%         testY(ind,1) = -1;      %把testY中为-2的改为-1.
       ...
       Spei(s,:) = TNi(s,:) / (FPi(s,:) + TNi(s,:)); %#修正spe指标实验
...
%      AUCi{s} = plot_roc(Mean_scores{s,1}(:, 2),testY);  % Mean_scores{s,2}要注意输入的哪一列的概率值。
    ...
    %% 决策层融合,下面部分使用网格权重搜索方法，集成三个子空间的预测
       % 加载权重矩阵
...
% function [f,U1,mode2,indx,missed_samples,missed_labels] = LDPP_Relief_adbstLDPP(trainX,trainY,validX,validY,valid_data,type_num)
% 
% kk=5;
% mukgamma=[];
% mean_svml8_max=0;
...
% 
% %% 查找错分样本
% trainZ3 = trainZ1(:,f);;   %在排列好序的特征中选出权重较大的特征进行实验，依次选出进行试验。
% validZ3 = validZ1(:,f);;
% % 训练
% %svm 线性
% % mode2 = svmtrain(trainY,trainZ3,'-s 0 -t 0');
% % svm_pred3 = svmpredict(validY,validZ3,mode2);
% 
% % SVM 高斯
% mode2 = svmtrain(trainY,trainZ3,'-s 0 -t 2');
% svm_pred3 = svmpredict(validY,validZ3,mode2);
% 
% % rf
% %  mode2 = classRF_train(trainZ3,trainY,'ntree',300)
% %  [svm_pred3,votes] = classRF_predict(validZ3,mode2)
%  
%  % 找出每一次训练测试后的错分样本
...
%% ldppB/ not_t  第二种方法 
function [f,U1,mode2,indx,missed_samples,missed_labels] = LDPP_Relief_adbstLDPP1(trainX,trainY,validX,validY,valid_data,type_num)
...
%         method.weightmode = 'binary';
        ...
        U = featureExtract2(trainX,trainY,method,type_num); %使用之前的方法提取出来映射矩阵U
           for ik = 1:floor(size(trainX,2)/3)
                method.K = kk * ik;
                mukgamma = [mukgamma;[imu ik igamma]];    
                trainZ1 = projectData(trainX, U, method.K);
                testZ1 = projectData(validX, U, method.K);
                
%                 % SVM高斯 
%                 model = svmtrain(trainY,trainZ1,'-s 0 -t 2');%%使用所有变换后的训练集训练模型
%                 svm_pred = svmpredict(validY,testZ1,model);  % 这里的testZ还是valid
...
[ACC_max,index] = max( svml7);
best_index = kk * index;
best_fea = fea(:,1:best_index );

%% 查找错分样本
trainZ3 = trainZ1(:,best_fea);  %在排列好序的特征中选出权重较大的特征进行实验，依次选出进行试验。
validZ3 = validZ1(:,best_fea);

% 训练
%svm 线性
% model2 = svmtrain(trainY,trainZ3,'-s 0 -t 0');
% svm_pred3 = svmpredict(validY,validZ3,model2);

% SVM 高斯
% model2 = svmtrain(trainY,trainZ3,'-s 0 -t 2');
% svm_pred3 = svmpredict(validY,validZ3,model2);

% rf
 model2 = classRF_train(trainZ3,trainY,'ntree',300)
 [svm_pred3,votes] = classRF_predict(validZ3,model2)
