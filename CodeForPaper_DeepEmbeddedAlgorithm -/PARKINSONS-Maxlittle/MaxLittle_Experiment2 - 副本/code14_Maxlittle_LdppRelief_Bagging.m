...
%% ldppB 
% function [f,U1,mode2,indx,missed_samples,missed_labels] = LDPP_Relief_Bagging_adbstLDPP(trainX,trainY,validX,validY,valid_data,type_num)
% kk = 2;
% mukgamma=[];
% mean_svml8_max=0;
% for igamma=1:9
...
% function [f,U1,mode2,indx,missed_samples,missed_labels] =LDPPH_not_Relief_Bagging_adbstLDPP(trainX,trainY,validX,validY,valid_data,type_num)
% kk = 4;
% mukgamma=[];
% mean_svml8_max=0;
% for igamma=1:9
...
% svml7 = [];
% 
% for p = 1:floor(size(trainZ1,2)/4)  % size(train,2) 计算train矩阵的列数
%     K =  p * kk;
%     trainZ2 = trainZ1(:,fea(:,1:K));   %在排列好序的特征中选出权重较大的特征进行实验，依次选出进行试验。
%     validZ2 = validZ1(:,fea(:,1:K)); 
%     %到了这里 数据集先试用LDPP ，在使用relief. 对LDPP变换出来的新特征在进行特征选择
