function [LSVTLDPPB_Relief_Result] = LSVTLDPPB_Relief_Result()
    clear all; 
    close all; 
    clc; warning('off');
    ...
    for iter=1:1
        ...
        %%评价标准
         ...
         %% 每一个子空间的评价指标
               ...
               Spei(s,:) = TNi(s,:) / (FPi(s,:) + TNi(s,:)); %# 1月6日调整 spe 指标
    ...
    for q = 1:m_w
      ...
      % best_Pred 是预测标签，testY是真实标签。到了这一步之后，预测标签和真实标签都为1和-1，原本的2都转为-1了。
           ...
           Pre = TP / (TP + FP);
           Rec = TP / (TP + FN);
           Spe = TN / (FP + TN);   %# 1月6日更正spe 指标
           
           G_mean = sqrt(Rec * Spe);
           F1_score_Final = (2 * Pre * Rec) / (Pre + Rec)
           tr2 = [tr2;ACC_svml_tt]; 
           ACC_final_mean = mean(tr2); 
           LSVTLDPPB_Relief_Result.ACC_final_mean = ACC_final_mean;
           LSVTLDPPB_Relief_Result.ACC = ACC;   
           LSVTLDPPB_Relief_Result.ACCi = ACCi; 
           LSVTLDPPB_Relief_Result.Pre = Pre;
           LSVTLDPPB_Relief_Result.Prei = Prei; 
           LSVTLDPPB_Relief_Result.Rec = Rec; 
           LSVTLDPPB_Relief_Result.Reci = Reci; 
           LSVTLDPPB_Relief_Result.Spe = Spe; 
           LSVTLDPPB_Relief_Result.Spei = Spei; 
            LSVTLDPPB_Relief_Result.G_mean = G_mean; 
           LSVTLDPPB_Relief_Result.G_meani = G_meani; 
           LSVTLDPPB_Relief_Result.F1_score_Final = F1_score_Final; 
           LSVTLDPPB_Relief_Result.F1_scorei = F1_scorei; 
