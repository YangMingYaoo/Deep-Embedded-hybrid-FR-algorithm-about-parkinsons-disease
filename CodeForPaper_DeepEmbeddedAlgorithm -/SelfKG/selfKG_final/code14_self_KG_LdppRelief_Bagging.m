...
%            validY = trainY0(size(trainX,1)+1:size(trainX0,1),:)
    ...
    for z = 1 : iter
        a = []
        [fea,U,mode2,indx,misX,misY] =  LDPP_Relief_Bagging_adbstLDPP(trainX1,trainY1,validX,validY,type_num)
        Us{z}=U;
        feature{z,1}=fea;
        Medels{z,1} = mode2;
...
%    Final_Evaluation_index = [ACC Pre Rec Spe G_mean  F1_score_Final  AUC_final ] 
   ...
   LdppRelief_Bagging_Result.TNR = TNR;
   LdppRelief_Bagging_Result.TNRi = TNRi
...
function [f,U1,mode2,indx,missed_samples,missed_labels] = LDPP_Relief_Bagging_adbstLDPP(trainX,trainY,validX,validY,type_num)
kk = 5;
mukgamma=[];
mean_svml8_max=0;
for igamma=1:9
