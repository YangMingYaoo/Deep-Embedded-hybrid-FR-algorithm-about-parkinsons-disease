...
%             %10折交叉验证数据集，每一折数据都输入进入一次，每一折数据输入后迭代3次，得到3个权重。
...
% for o=1:2;
    for p = 1:size(Trainingdata)
        ...
        for n = 1:nfold          
            ...
            trnError(n, :) = abClassifier.trnErr;
            tstError(n, :) = abClassifier.tstErr;
            W = [W abClassifier.Weight];
            abClassifierError(n) = sum(abClassifier.tstErr)/iter;
        end
%         A =  [A abClassifier.Weight]; %三次迭代后的 分类器权重
           [minabClassifierError,index]  = min(abClassifierError)
           A = W(:,index);
%          A =  abClassifier.Weight;
         B = sum(A)/3;            % 对三次迭代后的分离器权重进行求平均値。
        alpha(p)=B; 
