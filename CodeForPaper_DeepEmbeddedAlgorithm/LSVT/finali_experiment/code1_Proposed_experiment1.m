...
iter = 1
for C = 1:iter
    [MaxLittlerProposed_experiment1_s2] = LSVT_Proposed_experiment1_()
...
function [ LSVT_Proposed_experiment1_Result ] = LSVT_Proposed_experiment1_()
...
scores = [];
ensemble = [];
LSVT_Proposed_experiment1_Result = [];
...
%        Trainingdata=cell(iter,1); % For saving the training data from each iteration to calculate the corresponding Alpha
       ...
       for z = 1:iter
           ...
           [fea,U,model,indx,misX,misY] = ProposedB_adbstLDPP(trainX1,trainY1,validX,validY,type_num);
           Us{z}=U;
...
%          Final_Accuracy_with_Adaboosti = calculateAccuracy(test_data,testY,Result);
       ...
       FinalAcci = [FinalAcci Final_Accuracy_with_Adaboost];
...
%    TNR = TN / (FN + TN)
   ...
   tr2=[tr2;ACC_svml_tt];   
   LSVT_Proposed_experiment1_Result.ACC = ACC;   
   LSVT_Proposed_experiment1_Result.ACCi = ACCi; 
   LSVT_Proposed_experiment1_Result.Pre = Pre;
   LSVT_Proposed_experiment1_Result.Prei = Prei
   LSVT_Proposed_experiment1_Result.Rec = Rec; 
   LSVT_Proposed_experiment1_Result.Reci = Reci; 
   LSVT_Proposed_experiment1_Result.Spe = Spe; 
   LSVT_Proposed_experiment1_Result.Spei = Spei;
   LSVT_Proposed_experiment1_Result.G_mean = G_mean; 
   LSVT_Proposed_experiment1_Result.G_meani = G_meani; 
   LSVT_Proposed_experiment1_Result.F1_score= F1_score; 
   LSVT_Proposed_experiment1_Result.F1_scorei = F1_scorei; 
end
   end
...
function [f,U1,mode2,indx,missed_samples,missed_labels]= ProposedB_adbstLDPP(trainx,trainy,validx,validy,type_num)
...
%         testZ = projectData(validx, U, best_svml_kk);
        ...
        %% 使用relief特征选择算法
         ...
         % 下面代码 依次增加特征列数进行训练并预测精度  
          for ik = 1:floor(size(trainZ1,2)/4) 
            K = kk * ik;  
