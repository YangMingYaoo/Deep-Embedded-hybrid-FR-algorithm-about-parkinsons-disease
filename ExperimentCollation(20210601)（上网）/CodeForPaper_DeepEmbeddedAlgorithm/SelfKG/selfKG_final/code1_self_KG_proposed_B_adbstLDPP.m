...
iter = 10
for C = 1:iter
    [Result] = Proposed_B_experiment1svm0()
    final_eval{C} = Result;
end
...
function [ Proposed_H_Result ] = Proposed_B_experiment1svm0()
clc;
clear all;
close all;
warning('off');
...
scores = [];
ensemble = [];
self_KG_Proposed_B_experiment1_svm0 = [];
...
%         P = ensemble(:,1) * w1(1,1)+ ensemble(:,2) * w1(1,2) + ensemble(:,3) * w1(1,3);
   ...
   tr2=[tr2;ACC_svml_tt];   
   self_KG_Proposed_B_experiment1_svm0.ACC = ACC;   
   self_KG_Proposed_B_experiment1_svm0.ACCi = ACCi;
   self_KG_Proposed_B_experiment1_svm0.Pre = Pre;
   self_KG_Proposed_B_experiment1_svm0.Prei = Prei; 
   self_KG_Proposed_B_experiment1_svm0.Rec = Rec; 
   self_KG_Proposed_B_experiment1_svm0.Reci = Reci;
   self_KG_Proposed_B_experiment1_svm0.Spe = Spe; 
   self_KG_Proposed_B_experiment1_svm0.Spei = Spei;
   self_KG_Proposed_B_experiment1_svm0.G_mean = G_mean; 
   self_KG_Proposed_B_experiment1_svm0.G_meani = G_meani; 
   self_KG_Proposed_B_experiment1_svm0.F1_score = F1_score; 
   self_KG_Proposed_B_experiment1_svm0.F1_scorei = F1_scorei;
...
%       U_svml_best=U1_all;
       ...
       %% 上面代码找出最好的U矩阵参数存放在 method中通
        ...
        %# 使用最好的U矩阵进行变换
        trainZ1 = trainx * U(:,1:best_svml_kk); 
...
%               svm_pred2 = svmpredict(validy,validZ2,model,'-b 1');
%               svml2 = [svml2 mean(svm_pred2 == validy) * 100];
...
%              svml2 = [svml2 mean(svm_pred2 == validy) * 100]; 
                ...
                model = svmtrain(trainy,trainZ,'-s 0 -t 2 -b 1');%%使用所有变换后的训练集训练模型
                svm_pred2 = svmpredict(validy,validZ,model,'-b 1');  % 这里的testZ还是valid
                svml2 = [svml2 mean(svm_pred2 == validy) * 100];
...
%               svml2 = mean(svm_pred2 == validy) * 100;
